...
# pvalores_df: tabla generada por pvalJack()
# pls_model@vipVn: VIPs del modelo
...
```{r}
# Suponiendo que cada fila tiene exactamente un 1
Y_factor <- apply(Y, 1, function(row) colnames(Y)[which.max(row)])
Y <- factor(Y_factor)
...
library(caret)
library(e1071)   # Usamos naiveBayes de e1071
library(dplyr)

# 1. Definir Y e X (solo numéricas)
Y <- as.factor(data$X1ª_eval)
...
# 2. Preprocesado: centrar y escalar
X_scaled <- scale(X)

# 3. Leave-One-Out manual con naiveBayes (e1071)
set.seed(123)
n <- nrow(X_scaled)
preds <- character(n)
...
for (i in 1:n) {
  ...
  modelo_i <- naiveBayes(x = X_train, y = Y_train, laplace = 1)

  pred_i <- predict(modelo_i, X_test)
  preds[i] <- as.character(pred_i)
}

# 4. Matriz de confusión
confusionMatrix(factor(preds, levels = levels(Y)), Y)
```

```{r}
library(dplyr)

# 1. Definir Y como factor
Y <- as.factor(data$X1ª_eval)

# 2. Eliminar columnas "X1ª_eval" y "Mejor_resp", y quedarnos solo con numéricas
X <- data[, !(names(data) %in% c("X1ª_eval", "Mejor_resp"))]
X <- X[, sapply(X, is.numeric)]

# 2. Filtrar variables con p < 0.06 (ANOVA univariado)
pvals <- apply(X, 2, function(x) {
  summary(aov(x ~ Y))[[1]][["Pr(>F)"]][1]
})
vars_significativas <- names(pvals)[pvals < 0.06]
X_sig <- X[, vars_significativas, drop = FALSE]

# 3. Escalar
X_sig_scaled <- scale(X_sig)
...
for (i in 1:n) {
  X_train <- X_sig_scaled[-i, , drop = FALSE]
  Y_train <- Y[-i]
  X_test  <- X_sig_scaled[i, , drop = FALSE]
...
# 1. Preparar datos
Y <- as.factor(data$`X1ª_eval`)  # Clases: PE, EE, RP
...
pred_fun <- function(model, newdata) {
  ...
  # Devolver probabilidad para una clase específica, por ejemplo "RP"
  preds[, "RP", drop = FALSE]
}
