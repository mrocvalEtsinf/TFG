...
```{r}
library(caret)
library(e1071)  # Backend de Naive Bayes
...
# 2. Control de validación cruzada Leave-One-Out
ctrl_loo <- trainControl(
  method = "LOOCV",
  classProbs = TRUE,
  savePredictions = "final"
...
modelo_nb <- train(
  ...
  trControl = ctrl_loo,  # o ctrl_kfold_smote si usas SMOTE y CV
  preProcess = c("center", "scale"),  # escalar si es necesario
  tuneGrid = expand.grid(usekernel = TRUE, fL = 1, adjust = 1)
)
...
confusionMatrix(modelo_nb$pred$pred, modelo_nb$pred$obs)
```

```{r}
# Evaluar p-values por variable usando test univariado
...
confusionMatrix(modelo_nb_sig$pred$pred, modelo_nb_sig$pred$obs)
```
