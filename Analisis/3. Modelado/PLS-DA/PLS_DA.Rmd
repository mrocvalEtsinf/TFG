...
pvalJack <- function(pls, X, Y){
  ...
  # Escalar X y Y como en el modelo original
  X <- scale(X, center = pls@xMeanVn, scale = pls@xSdVn)
  Y <- scale(matrix(Y, ncol = 1), center = pls@yMeanVn, scale = pls@ySdVn)
  
  for (i in 1:nrow(X)) {
    
    pls_i <- suppressWarnings(ropls::opls(
      X[-i, , drop = FALSE],
      Y[-i, , drop = FALSE],
      scaleC = "none",
      predI = k,
    ...
    # Asegurar que los coeficientes tengan el mismo número de filas
    if (nrow(pls_i@coefficientMN) < nrow(coefmod)) {
      exclvar <- setdiff(rownames(coefmod), rownames(pls_i@coefficientMN))
      b <- matrix(0, ncol = 1, nrow = length(exclvar))
      rownames(b) <- exclvar
      pls_i@coefficientMN <- rbind(pls_i@coefficientMN, b)
    }
    
    orden <- match(rownames(coefmod), rownames(pls_i@coefficientMN))
    plscoefs_i <- pls_i@coefficientMN[orden, ]
    a <- cbind(a, plscoefs_i)
  }
  
  # Estadísticos Jackknife
...
Y_num <- as.numeric(Y)
pls_vip_num <- opls(x = X_vip, y = Y_num, crossvalI = nrow(X_vip),
                    scaleC = "standard", predI = 2)
pvalores <- pvalJack(pls = pls_vip_num, X = X_vip, Y = Y_num)
round(pvalores, 4)
```
...
```{r}
# Convertir lista a data.frame correctamente
pvalores_df <- data.frame(
  variable = pvalores$variable,
  pvalue = unlist(pvalores[-length(pvalores)])  # Quitamos el último elemento que es el nombre de variables
)

# Hacer merge con vip_df
vip_pval_df <- merge(vip_df, pvalores_df, by = "variable")

# Filtrar variables descartables: alta VIP pero p > 0.3
descartables <- vip_pval_df %>%
...
# 2. Variables seleccionadas (VIP > 1 y p-valor < 0.05)
vars_finales <- c(
  "Lymphocytes_media", "Histology_Escamoso", "Lymphocytes_diff_1C_2C",
  "Age_at_diagnosis", "Histology_Otros", "ALI_pre", "PCR",
  "Hemoglobin_diff_2C_1eval", "Platelets_cv", "Leukocytes_media",
  "Total_cholesterol", "PNI_diff_0_1C", "Hemoglobin_diff_0_1C",
  "Platelets_media", "Cardiopathy", "Albumin_slope"
)
...
# 4. Inicializar vector de predicciones
mypred <- character(nrow(X))
names(mypred) <- rownames(X)

# 5. LOO manual con tu script original
for (i in 1:nrow(X)) {
  X_train <- X[-i, , drop = FALSE]
  Y_train <- Y[-i]

  # Modelo PLS-DA
  pls_model <- opls(X_train, Y_train, predI = 2, orthoI = 0, fig.pdfC = "none", info.txtC = "none")
